 ...
 *
 * V1 交付 SQLite 一条路径：
 * - 桌面端：Tauri 侧 rusqlite 适配器（见 packages/desktop/src-tauri）；
 * - Node / 测试：node:sqlite 适配器（`@taskora/engine/node`）；
 * - Web（WASM + OPFS）为后续阶段预留。
 *
 * 接口刻意窄化为「带位置参数的 SQL」：任意能执行 SQLite 语句的后端
 * （本进程、Tauri IPC）都可实现。
 */
...
/** 事务辅助：异常自动回滚。存储实现不需要原生事务支持。 */
export function inTransaction<T>(storage: SqlStorage, fn: () => T): T {
  storage.exec('BEGIN');
  try {
    const result = fn();
    storage.exec('COMMIT');
